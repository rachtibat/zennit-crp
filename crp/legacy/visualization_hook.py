...
import gc
from crp.attribution_hook import CondAttribution
...
class FeatureVisualization:
    ...
    def run_analysis(self, composite, data_start, data_end, batch_size=32, checkpoint=500):
        """
        ...
        for b in range(batches):
            ...
            targets_samples, data_returned = [], []
            for index in samples_batch:

                single_data, single_label = self.get_data_sample(index)
                data_returned.append(single_data)
                targets_samples.append(single_label)
                data_batch = torch.stack(data_returned, dim=0)
            ...
            #TODO: add lock here because composite is not thread safe
            #data_broadcast.requires_grad = True
            ...
            def generate_hook(layer_name):
                ...
                def backward_hook(grad):
                    ...
                    self.analyze_relevance(grad.detach(), layer_name, concept, sample_indices, targets)

                def get_tensor_hook(module, input, output):
                    
                    concept = self.layer_map[layer_name]
                    self.analyze_activation(output.detach(), layer_name, concept, sample_indices, targets)

                    handle_inside.append(output.register_hook(backward_hook))

                return get_tensor_hook

            self.attribution(data_broadcast, conditions, composite, record_layer, append_hook=generate_hook)

            [h.remove() for h in handle_inside]
    ...
    def collect_results(self, data_start: int, data_end: int, checkpoints: Dict[str, List[str]]):
        ...
        self.MaxAct.collect_results(data_start, data_end, checkpoints["max"])
        self.MaxRel.collect_results(data_start, data_end, checkpoints["max"])
        self.RelStats.collect_results(data_start, data_end, checkpoints["stats"])
