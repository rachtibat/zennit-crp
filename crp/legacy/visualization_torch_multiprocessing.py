...
class FeatureVisualization:
    ...
    def run_analysis(self, composite, data_start, data_end, batch_size=32, checkpoint=500, worker=1):

        mp.set_start_method("spawn", True)

        self.attribution.model.share_memory()

        manager = mp.Manager()
        saved_files = manager.dict()
        lock = mp.Lock()

        jobs = []
        for i in range(worker):
            p = mp.Process(target=self.run_process, args=(composite, data_start, data_end, saved_files, lock, batch_size, checkpoint))
            jobs.append(p)
            p.start()
        ...
        return saved_files.copy()


    def run_process(self, composite, data_start, data_end, return_dict, lock: mp.Lock, batch_size=32, checkpoint=500):
        """
        ...
        for b in range(batches):
            ...
            # Zennit is not thread-safe
            with lock:
                _, activations, relevances = self.attribution(data_broadcast, conditions, composite, record_layer)
        ...
        return_dict.update(self.saved_checkpoints)
